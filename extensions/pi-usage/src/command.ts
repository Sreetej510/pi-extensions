			...
			try {
				...
				if (options.value.consumeBankedReset) {
					...
					if (!resetId) {
						...
						if (!selected) return;
						resetId = resets.credits.find(
							(credit) => formatCodexResetCreditChoice(credit) === selected,
						)?.id;
					}
					if (!resetId) {
				...
				let cached =
					combined && Date.now() - combined.createdAt < CACHE_TTL_MS ? combined : undefined;
				if (!cached) {
					...
					if (shared) {
						const entries = Object.values(shared.entries).filter(
							(entry): entry is SharedCacheEntry =>
								!!entry && Date.now() - entry.createdAt < CACHE_TTL_MS,
						);
						if (entries.length > 0) {
							cached = {
				...
				void queryAllUsage(ctx, cmdOptions)
					.then((result) => {
						...
						const kept = cmdOptions.statusline
							? applyCurrentProviderStatusline(ctx, result.reports)
							: false;
