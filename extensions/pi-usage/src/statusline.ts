...
export function setCombinedCache(
	value: { createdAt: number; reports: UsageReport[] } | undefined,
): void {
	combinedCache = value;
}
...
export const rethrowUnlessStaleContextError =
	(ctx: ExtensionContext) => (error: unknown) => {
...
export async function refreshCurrentUsageStatusline(
	ctx: ExtensionContext,
	...
	if (backoffRemaining > 0) {
		if (cached) {
			...
			setStatuslineValue(ctx, `usage rate-limited (${formatAgeShort(backoffRemaining)})`)
		) {
	...
	if (!result.ok) {
		...
		const errorLabel = rateLimited
			? `usage rate-limited (${formatAgeShort(retryDelayMs)})`
			: "usage error";
