...
import { getSupportedThinkingLevels, loadEnabledModelRefs, loadReviewConfig, saveReviewConfig, splitProviderModel } from "./config.js";
import { snapshotGitHead } from "./git.js";
import { PROGRESS_WIDGET_KEY, renderProgressLines } from "./progress.js";
import { loadExistingReport, mergeReport, buildRunSummary, REQUIRED_FILES } from "./report.js";
...
import type { CommandOption, GapStageResult, ReviewReport, ReviewerRole, TestGapCandidate, TestGapFinal, ThinkingLevel } from "./types.js";
...
const COMMAND_COMPLETIONS: readonly CommandOption[] = [
	...
	{ value: "--review", label: "--review", description: "Run only the 3 focus reviewer agents (description/tests/solution)" },
	{ value: "--description", label: "--description", description: "Run only the problem-description (prompt) focus reviewer" },
	...
	{ value: "--gap-finder", label: "--gap-finder", description: "Run only the 2-step behavioral test-gap finder + strict filter agents" },
...
async function runConfigFlow(ctx: ExtensionCommandContext) {
	...
	if (supportedLevels.length > 1) {
		...
		const levelOptions = [...supportedLevels].sort((a, b) => ((a === currentLevel) === (b === currentLevel) ? 0 : a === currentLevel ? -1 : 1));
	...
	ctx.ui.notify(`checks config saved: ${selected.parsed.provider}/${selected.parsed.modelId} (thinking: ${thinkingLevel})`, "info");
}
...
export function registerChecksCommand(pi: ExtensionAPI) {
	pi.registerCommand("checks", {
		...
		handler: async (args, ctx) => {
			...
			if (unknown.length > 0) {
				ctx.ui.notify(`Unknown option(s): ${unknown.join(", ")}. Run /checks with no arguments to see the available options.`, "warning");
				return;
			}
			...
			if (!runReviewers && !runGapStages) {
				ctx.ui.notify(`Nothing to run for: ${tokens.join(" ")}. Run /checks with no arguments to see the available options.`, "warning");
				return;
			}
			...
			if (!model) {
				ctx.ui.notify(`Configured model ${config.provider}/${config.modelId} not found. Run /checks --config again.`, "error");
				return;
			}
