...
import type { ChecksConfig, ReviewReport, ReviewerRole, TestGapFinal, Verdict } from "./types.js";
...
export function mergeReport(input: MergeReportInput): Record<string, unknown> {
	const { existingReport, config, runReviewers, byRole, runGapStages, testGaps, gapAnalysisIncomplete, gapFinderStatus, gapFilterStatus } =
		input;
...
export function buildRunSummary(opts: {
	...
	const roleVerdicts: Record<string, Verdict> | undefined = runReviewers
		? Object.fromEntries(activeRoles.filter((role) => byRole[role.key]).map((role) => [role.key, byRole[role.key].verdict]))
		: undefined;
